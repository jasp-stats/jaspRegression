...
#### Plots
- Scatter plots: Displays scatter plots for all variable pairs. In a matrix format, these are placed above the diagonal. For partial correlations, plots show the relationship between variables $X$ and $Y$ after removing the effects of control variables $Z_1, Z_2, ..., Z_n$, as specified in the input menu. In that case, axes of each plot depict the residuals from regressing $X$ and $Y$ on $Z_1, Z_2, ..., Z_n$.
