...
Form
{
		...
		Group
		{
			...
			CheckBox
			{
				...
				CheckBox
				{
					...
					label: qsTr("From")
					info: qsTr("Apply bootstrapped estimation. By default, the number of replications is set to 1000, but this can be changed into the desired number.")
			...
			CheckBox
			{
				name: "coefficientCi"; label: qsTr("Confidence intervals"); info: qsTr("Include confidence intervals for the estimated mean difference. By default the confidence level is set to 95%, but this can be changed into the desired percentage.")
				childrenOnSameRow: true
				CIField { name: "coefficientCiLevel" }
			}
			CheckBox { name: "collinearityStatistic";		label: qsTr("Tolerance and VIF"); info: qsTr("Display Tolerance and Variance Inflation Factor for each predictor in the model in order to assess multicollinearity.")		}
	...
	Section
	{
		...
		RadioButtonGroup
		{
			...
			RadioButton
			{
				value: "pValue"; label: qsTr("Use p value"); info: qsTr("Use p-value as criterion for adding and removing predictors in Backward, Forward, and Stepwise regression. Entry adds  predictor if p-value of regression coefficient < x; default is x=0.05. Removal Removes predictor if p-value of regression coefficient > x; default is x=0.1.") ; checked: true
				columns: 2
